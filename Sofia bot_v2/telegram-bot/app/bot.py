    ...
    async def stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with AsyncSessionLocal() as db:
            ...
            for i, stats in enumerate(all_stats[:10], 1):
                ...
                history = await stats_service.get_chat_history(stats["chat_id"], days=7)
                trend = await stats_service.calculate_trend(stats["chat_id"])
                ...
                }.get(trend["trend"], "недостаточно данных")
                ...
                if trend["trend"] != "unknown":
                    message_lines.append(
                        f"   📈 Тренд: {trend['direction']} {trend['change_percent']:+.1f}% "
                        f"({trend_text})"
                    )
                else:
                    message_lines.append(f"   📈 Тренд: недостаточно данных")
                
                if history:
                    message_lines.append("   История:")
                    for h in history:
                        score_str = f"{h['score']:.0f}%" if h["score"] else "---"
                        message_lines.append(
                            f"   {h['weekday']}: {h['bar']} {score_str}"
                        )
